...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        storage_configs = [
            ...
            # ("GRAPH_STORAGE", self.graph_storage),  # 已禁用知识图谱存储
        ...
        # 禁用知识图谱存储 - 使用默认的NetworkXStorage作为占位符
        ...
        # 禁用知识图谱存储 - 注释掉graph_storage_cls的partial配置
        ...
        #     embedding_func=self.embedding_func,
        # )
        self.chunk_entity_relation_graph = None  # 纯文档存储模式不需要图存储
    ...
    async def check_and_migrate_data(self):
        """Check if data migration is needed and perform migration if necessary - 知识图谱功能已禁用"""
        if self.chunk_entity_relation_graph is None:
            logger.debug("Knowledge graph is disabled, skipping migration check")
            return
    ...
    async def _migrate_entity_relation_data(self, processed_docs: dict):
        """Migrate existing entity and relation data to full_entities and full_relations storage - 知识图谱功能已禁用"""
        if self.chunk_entity_relation_graph is None:
            logger.debug("Knowledge graph is disabled, skipping migration")
            return
    ...
    async def get_knowledge_graph(
        ...
        """Get knowledge graph for a given label - 知识图谱功能已禁用
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            # 移除知识图谱构建阶段 - 只保留向量数据库存储，不再构建实体关系图
                            ...
                            async with pipeline_status_lock:
                                ...
                                all_tasks = []
                                if 'first_stage_tasks' in locals():
                                    all_tasks.extend(first_stage_tasks)
                                # 移除知识图谱任务的取消逻辑
                                # if 'entity_relation_task' in locals() and entity_relation_task:
                                #     all_tasks.append(entity_relation_task)
                            ...
                            await self.doc_status.upsert(
                                {
                                    doc_id: {
                                        ...
                                        "track_id": status_doc.track_id,  # Preserve existing track_id
                                        "metadata": {
                                            "processing_start_time": processing_start_time if 'processing_start_time' in locals() else int(time.time()),
        ...
        try:
            ...
            async with graph_db_lock:
                ...
                try:
                    # 检查知识图谱是否被禁用 - 跳过图数据处理
                    if self.chunk_entity_relation_graph is None:
                        logger.info("知识图谱功能已禁用，跳过实体和关系处理")
                    else:
                        # Process entities
                ...
                if entities_to_delete and self.chunk_entity_relation_graph is not None:
                    try:
                ...
                if relationships_to_delete and self.chunk_entity_relation_graph is not None:
                    try:
                        # Delete from vector database
                        rel_ids_to_delete = []
                ...
                if (entities_to_rebuild or relationships_to_rebuild) and self.chunk_entity_relation_graph is not None:
                    try:
