...
async def _rebuild_knowledge_from_chunks(
    ...
    # Execute all tasks in parallel with improved error handling and cancellation protection
    try:
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                if isinstance(result, asyncio.CancelledError):
                    cancelled_tasks += 1
                    logger.debug(f"Task {i} was cancelled during rebuild process")
                else:
                    logger.error(f"Task {i} failed during rebuild: {result}")
        ...
        logger.info(f"Rebuild completed: {successful_tasks}/{total_tasks} successful, {cancelled_tasks} cancelled, {len(exceptions)} failed")
        
        # Only raise if all non-cancelled tasks failed
        if successful_tasks == 0 and exceptions:
            logger.error("All non-cancelled rebuild tasks failed, raising first exception")
            raise exceptions[0]
        elif successful_tasks > 0:
            logger.info(f"Rebuild partially successful with {successful_tasks} completed tasks")
            
    except asyncio.CancelledError:
        # Handle case where the entire gather operation is cancelled
        logger.warning("Rebuild operation was cancelled externally")
...
async def _merge_nodes_then_upsert(
    ...
    if knowledge_graph_inst is None:
        logger.info(f"Knowledge graph is disabled, skipping node merge for entity: {entity_name}")
        return
...
async def _merge_edges_then_upsert(
    ...
    # 检查知识图谱是否被禁用
    if knowledge_graph_inst is None:
        logger.info(f"Knowledge graph is disabled, skipping edge merge for {src_id} -> {tgt_id}")
        return None
...
async def merge_nodes_and_edges(
    ...
    # 检查知识图谱是否被禁用
    if knowledge_graph_inst is None:
        log_message = f"Knowledge graph disabled, skipping merge for {file_path}"
