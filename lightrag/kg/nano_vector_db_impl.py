...
class NanoVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.warning(f"Embedding tasks were cancelled for {self.namespace}, retrying with sequential processing")
            # Fallback to sequential processing if parallel tasks are cancelled
            embeddings_list = []
            for batch in batches:
                try:
                    ...
                    logger.error(f"Sequential embedding also cancelled for {self.namespace}, skipping batch")
                    # Create zero embeddings as fallback
                    batch_size = len(batch)
                    embedding_dim = getattr(self.embedding_func, 'embedding_dim', 1536)  # Default dimension
                    zero_embedding = np.zeros((batch_size, embedding_dim))
                    embeddings_list.append(zero_embedding)
                except Exception as e:
                    logger.error(f"Error in sequential embedding for {self.namespace}: {e}")
                    # Create zero embeddings as fallback
                    batch_size = len(batch)
                    embedding_dim = getattr(self.embedding_func, 'embedding_dim', 1536)  # Default dimension
                    zero_embedding = np.zeros((batch_size, embedding_dim))
