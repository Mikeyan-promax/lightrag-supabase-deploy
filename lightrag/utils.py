...
async def use_llm_func_with_cache(
    ...
    if llm_response_cache:
        ...
        # Call LLM with timeout and cancellation protection
        try:
            # Add timeout to prevent hanging LLM calls
            res: str = await asyncio.wait_for(
                use_llm_func(input_text, **kwargs), 
                timeout=300.0  # 5 minutes timeout
            )
            res = remove_think_tags(res)
        except asyncio.TimeoutError:
            logger.warning(f"LLM call timed out for input length: {len(input_text)}")
            raise
        except asyncio.CancelledError:
            logger.debug(f"LLM call was cancelled for input length: {len(input_text)}")
            raise
